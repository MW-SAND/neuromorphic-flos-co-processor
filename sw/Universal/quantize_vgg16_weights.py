...
This script quantizes the weights of a VGG16 model with quantized activations 
"""
...
def quantizeWeights(parameters):
    ...
    for idx, layer in enumerate(model.layers):
        if isinstance(layer, (tf.keras.layers.Conv2D, tf.keras.layers.Dense)):
            ...
            print(f"   First quantization: {percentile}th percentile = {p_value:.4f}, scaling factor = {scaling_factor:.4f}")
            ...
            # Update weights and disable training of the weights
            new_weights = [weights_quantized, bias_quantized]
            layer.set_weights(new_weights)
            layer.trainable = False

            # Adapt the scale factor of the activations to minimize impact of weight quantization on the activations
            if layer.name != "predictions":
                print(
                    f"layer_name: {model.layers[idx+1].name}, scale_factor: {model.layers[idx+1].scale_factor}")
            ...
            print(f"   Retraining entire model for 1 epochs after scaling layer {layer.name}...")

            # Retrain subsequent layers 
            epochs = 4
            history = model.fit(aug.flow(x_train, y_train, batch_size=batch_size), validation_data=(
                x_val, y_val), steps_per_epoch=len(x_train) // batch_size, epochs=epochs, callbacks=[reduce_lr])
...
if __name__ == "__main__":
    parameters = {
        "model_path": "VGG16_qA.model",           # path to your trained model
        "output_model_path": "VGG16_qW_nt_rlr.model",   # output quantized model filename
